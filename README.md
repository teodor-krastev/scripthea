...
The viewer shows a Scripthea image depot (a folder with a bunch of images and decription.txt file for the prompts). Organize and review generated images alongside their corresponding prompts. The viewer supports both grid and table views, aiding in visual analysis and comparison.
The image itself can be zoomed in/out(buttons), panned (scroll-bars), or fit (the middle button).
...
The last tab offers you automation of your workflow by using integrated Python scripting with access to many of Scripthea's features.
